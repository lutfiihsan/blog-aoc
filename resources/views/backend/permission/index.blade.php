            ...
            <div class="card">
                ...
                <div class="card-body px-6 pb-6">
                    <div class="overflow-x-auto -mx-6">
                                ...
                                <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                    ...
                                    <tbody
                                        ...
                                        @forelse ($permissions as $permission)
                                            <tr>
                                                <td class="table-td">{{ $permission->id }}</td>
                                                <td class="table-td">{{ $permission->name }}</td>
                                                <td class="table-td">{{ $permission->guard_name }}</td>
                                                <td class="table-td">{{ $permission->created_at->diffForHumans() }}</td>
                                                <td class="table-td">
                                                    @if ($permission->name == 'Super Admin')
                                                        <span
                                                            class="text-center text-muted"><i>{{ __('Pengguna default') }}</i></span>
                                                    @else
                                                        @can('ubah_role')
                        ...
                        <hr class="mt-4 mb-6 dark:border-slate-700">
                    </div>
                    {{ $permissions->links() }}
                </div>
            </div>
